...
class _SelectedMapScreenState extends State<SelectedMapScreen> {
                    ...
                    if (selectedSalonId != null && nearbySalons!.length == 1) {
                      ...
                      for (var salon in nearbySalons!) {
                        ...
                        if (salon is Map) {
                          ...
                          shopName = salon['shopName'] ?? 'Salon';
                          shopAddress = salon['shopAddress'] ?? '';
                          salonUserId = salon['saloonOwnerId'] ?? salon['userId'] ?? salon['id'] ?? '';
                          shopLogo = salon['shopLogo'] ?? '';
                        } else {
                          salonLat = salon.latitude ?? 0.0;
                          salonLng = salon.longitude ?? 0.0;
                      ...
                      if (!isFromShopProfile && nearbySalons!.isNotEmpty) {
                        ...
                        if (firstSalon is Map) {
                          salonUserId = firstSalon['saloonOwnerId'] ??
                              firstSalon['userId'] ??
                              firstSalon['id'] ??
                              '';
                        } else {
  ...
  Map<String, dynamic> _salonToMap(dynamic salon) {
    // Handle availableBarbers conversion
    List<Map<String, dynamic>>? availableBarbersList;
    if (salon.availableBarbers != null) {
      if (salon.availableBarbers is List) {
        availableBarbersList = salon.availableBarbers.map((barber) {
          if (barber is Map) {
            return Map<String, dynamic>.from(barber);
          } else {
            // Handle AvailableBarber object
            return {
              'barberId': barber.barberId ?? '',
              'barberName': barber.barberName ?? '',
              'barberImage': barber.barberImage ?? '',
              'queueCount': barber.queueCount ?? 0,
              'availableForQueue': barber.availableForQueue ?? false,
              'availableForBooking': barber.availableForBooking ?? false,
              'serviceType': barber.serviceType ?? '',
              'workingHours': barber.workingHours != null
                  ? {
                      'openingTime': barber.workingHours!.openingTime,
                      'closingTime': barber.workingHours!.closingTime,
                    }
                  : null,
            };
          }
    ...
    return {
      'shopId': salon.shopId ?? '',
      'saloonOwnerId': salon.saloonOwnerId ?? salon.userId ?? '',
      'userId': salon.userId ?? salon.saloonOwnerId ?? '',
      'shopName': salon.shopName ?? 'Salon',
      'shopAddress': salon.shopAddress ?? '',
      'shopLogo': salon.shopLogo ?? '',
      ...
      'availableBarbers': availableBarbersList ?? [],
  ...
  Widget _buildSalonBottomSheet(Map<String, dynamic> salonData) {
    ...
    final salonUserId = salonData['userId'] ?? salonData['saloonOwnerId'] ?? '';
    final availableBarbersCount = salonData['totalAvailableBarbers'] ?? 0;
    final availableBarbersList = (salonData['availableBarbers'] as List<dynamic>?)
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  SizedBox(height: 8.h),
                                  CustomText(
                                    text: availableBarbersCount.toString(),
                                    fontSize: 20.sp,
                                    fontWeight: FontWeight.bold,
                                    color: AppColors.secondary,
                                  ),
                      ...
                      // Available Barbers Section
                      if (availableBarbersList.isNotEmpty)
                        Padding(
                          padding: EdgeInsets.only(top: 24.h),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              ...
                              ...availableBarbersList.map((barber) {
                                final barberName = barber['barberName'] ?? 'Barber';
                                final barberImage = barber['barberImage'] ?? '';
                                final serviceType = barber['serviceType'] ?? '';
                                final availableForBooking = barber['availableForBooking'] ?? false;
                                final availableForQueue = barber['availableForQueue'] ?? false;
                                final queueCount = barber['queueCount'] ?? 0;
                                final workingHours = barber['workingHours'] as Map<String, dynamic>?;
                                          ...
                                          children: [
                                            ...
                                            Row(
                                              children: [
                                                ...
                                                if (availableForBooking || availableForQueue) ...[
                                                  SizedBox(width: 6.w),
                                                  Container(
                                            ...
                                            Row(
                                              children: [
                                                if (serviceType == 'QUEUE' && queueCount > 0) ...[
                                                  Icon(Icons.people,
                                                      size: 14.sp,
                                                      color: AppColors.gray300),
                                                  SizedBox(width: 4.w),
